...
async def rename_watchlist(wid: int, body: WatchlistUpdate, db: DbSession, user: CurrentUser) -> WatchlistSummary:
    wl = await _own_watchlist(db, wid, user.id)
    wl.name = body.name
    await db.commit()
    count = await db.scalar(
        select(sqlfunc.count()).select_from(WatchlistItem).where(WatchlistItem.watchlist_id == wid)
    ) or 0
    return WatchlistSummary(id=wl.id, name=wl.name, is_default=wl.is_default, created_at=wl.created_at, item_count=count)
...
@router.post("/{wid}/items/{symbol}", response_model=WatchlistOut, status_code=status.HTTP_201_CREATED)
async def add_item(wid: int, symbol: str, db: DbSession, user: CurrentUser) -> Watchlist:
    wl = await _own_watchlist(db, wid, user.id)
    if len(wl.items) >= MAX_ITEMS:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Watchlist full ({MAX_ITEMS} items)")
